...
public class BetterNautilusConfig {
    ...
    public static void load() {
        ...
        if (Files.exists(configFile)) {
            try {
                ...
                if (INSTANCE == null) {
                    BetterNautilus.LOGGER.warn(
                            "[BetterNautilus] Config file was empty or null — using defaults.");
                    INSTANCE = new BetterNautilusConfig();
                }
                INSTANCE.clamp();
                BetterNautilus.LOGGER.info("[BetterNautilus] Config loaded from {}", configFile);
            } catch (Exception e) {
                BetterNautilus.LOGGER.error(
     ...
     * attribute assignment (ClampedEntityAttribute will throw if base > max).
